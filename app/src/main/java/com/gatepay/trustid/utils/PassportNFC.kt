package com.gatepay.trustid.utils
...
private constructor() {
    ...
    constructor(ps: PassportService?, trustManager: MRTDTrustStore, mrzInfo: MRZInfo) : this() {
        ...
        if (hasBAC && !(hasSAC && isSACSucceeded)) {
            ...
            val triedBACEntries = java.util.ArrayList<BACKey>()
        ...
        val dgNumbers = java.util.ArrayList<Int>()
        if (sodFile != null) {
    ...
    private fun verifyAA() {
        ...
        try {
            ...
            if ("EC" == pubKeyAlgorithm || "ECDSA" == pubKeyAlgorithm) {
                ...
                val activeAuthenticationInfoList = java.util.ArrayList<ActiveAuthenticationInfo>()
    ...
    private fun verifyCS() {
        try {
            ...
            val chain = java.util.ArrayList<Certificate>()
    ...
    private fun doPACE(ps: PassportService, mrzInfo: MRZInfo): PACEResult? {
        ...
        try {
            ...
            val paceInfos = java.util.ArrayList<PACEInfo>()
            if (securityInfo is PACEInfo) {
    ...
    private fun doEACCA(ps: PassportService, mrzInfo: MRZInfo, dg14File: DG14File?, sodFile: SODFile?): List<EACCAResult> {
        ...
        //Chip Authentication
        val eaccaResults = java.util.ArrayList<EACCAResult>()

        var chipAuthenticationInfo: ChipAuthenticationInfo? = null

        val chipAuthenticationPublicKeyInfos =
            java.util.ArrayList<ChipAuthenticationPublicKeyInfo>()
        val securityInfos = dg14File.securityInfos
    ...
    private fun doEACTA(ps: PassportService, mrzInfo: MRZInfo, cvcaFile: CVCAFile?, paceResult: PACEResult?, eaccaResult: EACCAResult?, cvcaKeyStores: List<KeyStore>): List<EACTAResult> {
        ...
        val eactaResults = java.util.ArrayList<EACTAResult>()
    ...
    private fun toDataGroupList(tagList: IntArray?): List<Int>? {
        ...
        val dgNumberList = java.util.ArrayList<Int>(tagList.size)
        for (tag in tagList) {
            try {
